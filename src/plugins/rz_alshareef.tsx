...
const settings = definePluginSettings({
    ...
    onlyManualTrigger: {
        type: OptionType.BOOLEAN,
        description:
            "لا تلحق الفولو تلقائيًا، اشتغل فقط لما تضغط من الإنديكيتور / يدوي",
        restartNeeded: false,
        default: false
    },
    followLeave: {
    ...
    channelFull: {
        type: OptionType.BOOLEAN,
        description:
            "لا تحاول تدخل روم الفولو إذا هو فل (إلا لو عندك صلاحية Move Members)",
        restartNeeded: false,
        default: true
    },
    ...
    autoSelfUnmute: {
        type: OptionType.BOOLEAN,
        description:
            "فك الميوت والدفن عن نفسك تلقائيًا عند دخول روم (client mute/deaf)",
        default: true,
        restartNeeded: false
    },
    reconnect: {
    ...
    autoRolePullEnabled: {
        type: OptionType.BOOLEAN,
        description:
            "سحب أصحاب الرول المحدد تلقائيًا إلى رومك عند انتقالك لروم جديد",
        default: false,
        restartNeeded: false
    },
    // Auto Pull
    ...
    voiceTourDelaySeconds: {
        type: OptionType.SLIDER,
        description: "مدة التوقف في كل روم أثناء التمشية (ثواني)",
        default: 2,
        markers: makeRange(1, 10, 1)
    },
    voiceTourMaxSteps: {
        type: OptionType.SLIDER,
        description: "كم روم يمشي قبل ما يروح لروم عشوائي",
        default: 10,
        markers: makeRange(1, 50, 1)
    },
    voiceTourOffset: {
        type: OptionType.SLIDER,
        description: "كم روم تطلع فوق أو تنزل تحت (للتمشية اليدوية)",
        default: 10,
        markers: makeRange(1, 50, 1)
    },
    ...
    hotkeyClearAll: {
        type: OptionType.STRING,
        description:
            "اختصار يشيل كل شي (فولو / أبيّك / تجميد / بلاك / زحلقه / تثبيت / رول)",
...
function setFollow(userId: string, enable: boolean) {
    ...
    if (enable) {
        if (!ids.includes(userId)) {
            ...
            showRZMessage(`حطيت ${name} في الفولو`, "success");
        }
        ...
        if (ids.includes(userId)) {
            ...
            const u = UserStore.getUser(userId);
            const name = u?.username ?? userId;
            showRZMessage(`شلت الفولو عن ${name}`, "info");
        }

        if (settings.store.currentFollowId === userId) {
            const newIds = getFollowIds();
            const newCurrent = newIds.length ? newIds[newIds.length - 1] : "";
            settings.store.currentFollowId = newCurrent;
        }

        if (settings.store.followUserId === userId) {
            const newIds = getFollowIds();
            const newFollow = newIds.length ? newIds[newIds.length - 1] : "";
            settings.store.followUserId = newFollow;
        }
    }
}
...
function triggerFollow(targetUserId?: string | null) {
    ...
    if (!userChannelId || !guildId) {
        const fUser = UserStore.getUser(followId);
        const fName = fUser?.username ?? followId;
        showRZMessage(`${fName} مو في روم صوتي`, "info");
        return;
    }
    ...
    const fUser = UserStore.getUser(followId);
    const fName = fUser?.username ?? followId;
    showRZMessage(`لحقت ${fName}`, "success");
...
function addToPullList(userId: string) {
    ...
    if (!ids.includes(userId)) {
        ...
        const u = UserStore.getUser(userId);
        const name = u?.username ?? userId;
        showRZMessage(`حطيت ${name} في قائمة السحب`, "success");
    }
}
function removeFromPullList(userId: string) {
    ...
    const u = UserStore.getUser(userId);
    const name = u?.username ?? userId;
    showRZMessage(`شلت ${name} من قائمة السحب`, "info");
}
...
/* تمشية تتبّع */
let tourOriginalChannel: Record<string, string> = {};
let tourRunningUsers = new Set<string>();
...
function hasPermsBlacklist(
    channelId: string,
    need: "mute" | "deaf" | "move" | "any" = "any"
): boolean {
    try {
        const channel = ChannelStore.getChannel(channelId);
        if (!channel) return false;

        const canMute = PermissionStore.can(MUTE, channel);
        const canDeafen = PermissionStore.can(DEAFEN, channel);
        const canMove = PermissionStore.can(MOVE, channel);

        if (need === "mute") return canMute;
        if (need === "deaf") return canDeafen;
        if (need === "move") return canMove;
        return canMute || canDeafen || canMove;
    } catch {
        return false;
    }
}
        ...
        for (const entry of currentBlacklist) {
            ...
            for (const users of Object.values(allStates)) {
                ...
                if (userState && userState.channelId === channelId) {
                    const channel = ChannelStore.getChannel(channelId);
                    if (channel && hasPermsBlacklist(channelId, "any")) {
...
// تمشية في كل رومات السيرفر (تتوقف إذا أوقفت التمشية)
async function voiceTourWithReturn(
    guildId: string,
    userId: string,
    ...
    const voiceChannels = guildChannels.VOCAL.map(v => v.channel).filter(
        ch => ch.type === 2 || ch.type === 13
    );

    const delay = settings.store.voiceTourDelaySeconds * 1000;

    for (const ch of voiceChannels) {
        if (!tourRunningUsers.has(userId)) break;
        if (ch.id === startChannelId) continue;
        await moveUserToChannel(guildId, userId, ch.id);
        await sleep(delay);
    }
}

// تمشية بدون رجعة + بعد X رومات يروح روم عشوائي ويوقف
async function voiceTourNoReturnRandomEnd(
    guildId: string,
    userId: string,
    startChannelId: string
) {
    ...
    let voiceChannels = guildChannels.VOCAL.map(v => v.channel)
    ...
    const delay = settings.store.voiceTourDelaySeconds * 1000;
    const maxSteps = settings.store.voiceTourMaxSteps;
    ...
    voiceChannels = voiceChannels.filter(ch => ch.id !== startChannelId);
    if (!voiceChannels.length || !tourRunningUsers.has(userId)) return;

    const randomTarget =
        voiceChannels[Math.floor(Math.random() * voiceChannels.length)];

    await moveUserToChannel(guildId, userId, randomTarget.id);

    tourRunningUsers.delete(userId);
    delete tourOriginalChannel[userId];

    const u = UserStore.getUser(userId);
    showRZMessage(
        `تم تمشية ${u?.username ?? userId} وتحط في روم عشوائي (${randomTarget.name})`,
        "success"
    );
}

// روم عشوائي مرّة واحدة (بدون تمشية) لهذا الشخص
async function moveUserToRandomChannelOnce(user: User) {
    const startChannelId = getUserChannelId(user.id);
    if (!startChannelId) {
        showRZMessage("هذا الشخص مو في روم صوتي", "info");
        return;
    }

    const startChannel = ChannelStore.getChannel(startChannelId);
    if (!startChannel?.guild_id) {
        showRZMessage("ما قدرت أجيب السيرفر حق هذا الشخص", "error");
        return;
    }

    const guildId = startChannel.guild_id;
    const guildChannels: { VOCAL: { channel: Channel; comparator: number }[] } =
        GuildChannelStore.getChannels(guildId);
    ...
    if (!voiceChannels.length) {
        showRZMessage("ما فيه رومات صوتية ثانية في هذا السيرفر", "info");
        return;
    }

    const randomTarget =
        voiceChannels[Math.floor(Math.random() * voiceChannels.length)];
    await moveUserToChannel(guildId, user.id, randomTarget.id);
    showRZMessage(
        `تم نقل ${user.username} لروم عشوائي (${randomTarget.name})`,
        "success"
    );
}

// روم عشوائي مرّة واحدة عليك أنت من منيو الروم
async function moveMeToRandomChannelOnceFromChannelMenu(channel: Channel) {
    const myChannelId = SelectedChannelStore.getVoiceChannelId();
    const myChannel = myChannelId ? ChannelStore.getChannel(myChannelId) : null;

    if (!myChannelId || !myChannel) {
        showRZMessage("ادخل روم صوتي أول عشان أقدر أوديك لروم عشوائي", "info");
        return;
    }

    const guildId = myChannel.guild_id;
    if (!guildId) {
        showRZMessage("ما قدرت أجيب السيرفر حق رومك الحالي", "error");
        return;
    }
    ...
    voiceChannels = voiceChannels.filter(ch => ch.id !== myChannelId);
    if (!voiceChannels.length) {
        showRZMessage("ما فيه رومات صوتية ثانية في هذا السيرفر", "info");
        return;
    }
    ...
    if (!PermissionStore.can(CONNECT, randomTarget)) {
        showRZMessage(
            "ما عندك صلاحية تدخل الروم العشوائي اللي طلع معك",
            "error"
        );
        return;
    }

    ChannelActions.selectVoiceChannel(randomTarget.id);
    showRZMessage(
        `تم نقلك لروم عشوائي (${randomTarget.name})`,
        "success"
    );
}

/* تمشية + رجعة حقيقية لنفس رومه القديم */

async function tourUserWithReturn(user: User) {
    const currentChannelId = getUserChannelId(user.id);
    if (!currentChannelId) {
        showRZMessage("هذا الشخص مو في روم صوتي", "info");
        return;
    }
    ...
    if (!PermissionStore.can(MOVE, currentChannel)) {
        showRZMessage(
            "ما عندك صلاحية Move Members في روم هذا الشخص",
            "error"
        );
        return;
    }

    tourOriginalChannel[user.id] = currentChannelId;
    tourRunningUsers.add(user.id);

    await voiceTourWithReturn(guildId, user.id, currentChannelId);

    const original = tourOriginalChannel[user.id];
    if (original && tourRunningUsers.has(user.id)) {
        await moveUserToChannel(guildId, user.id, original);
    }

    tourRunningUsers.delete(user.id);
    delete tourOriginalChannel[user.id];

    showRZMessage(
        `تم تمشية ${user.username} ورجعته لنفس رومه القديم`,
        "success"
    );
}

// تمشية بدون رجعة + روم عشوائي نهائي
async function tourUserNoReturnRandom(user: User) {
    const startChannelId = getUserChannelId(user.id);
    if (!startChannelId) {
        showRZMessage("هذا الشخص مو في روم صوتي", "info");
        return;
    ...
    if (!PermissionStore.can(MOVE, startChannel)) {
        showRZMessage(
            "ما عندك صلاحية Move Members في روم هذا الشخص",
            "error"
        );
    ...
    tourRunningUsers.add(user.id);
    await voiceTourNoReturnRandomEnd(guildId, user.id, startChannelId);
}

/* تمشية فوق/تحت بعدد رومات معيّن */

function getOrderedVoiceChannelsForGuild(guildId: string) {
    const guildChannels: { VOCAL: { channel: Channel; comparator: number }[] } =
        GuildChannelStore.getChannels(guildId);

    return guildChannels.VOCAL.map(v => v.channel)
        .filter(ch => ch.type === 2 || ch.type === 13)
        .sort((a, b) => a.position - b.position);
}

async function moveUserOffset(user: User, direction: "up" | "down") {
    const startChannelId = getUserChannelId(user.id);
    if (!startChannelId) {
        showRZMessage("هذا الشخص مو في روم صوتي", "info");
        return;
    ...
    const list = getOrderedVoiceChannelsForGuild(guildId);
    const idx = list.findIndex(ch => ch.id === startChannelId);
    if (idx === -1) return;

    const offset = settings.store.voiceTourOffset;
    let targetIdx = direction === "up" ? idx - offset : idx + offset;
    ...
    await moveUserToChannel(guildId, user.id, target.id);
    showRZMessage(
        `تم نقل ${user.username} إلى روم (${target.name})`,
        "success"
    );
}
...
// ميوت سيرفري لشخص واحد
async function serverMuteUser(
    guildId: string,
    userId: string,
    mute: boolean
) {
...
const UserContext: NavContextMenuPatchCallback = (
    ...
    // تمشية + رجعة لنفس رومه
    groups.push(
        <Menu.MenuItem
            id="rz-user-tour-return"
            label="تمشية في رومات السيرفر ورجعة لنفس رومه"
            action={() => {
                void tourUserWithReturn(user);
            }}
        />
    );
    ...
    groups.push(
        <Menu.MenuItem
            id="rz-user-tour-random-end"
            label="تمشية ثم روم عشوائي ووقف"
            action={() => {
                void tourUserNoReturnRandom(user);
            }}
        />
    );

    // روم عشوائي مرّة واحدة لهذا الشخص
    groups.push(
        <Menu.MenuItem
            id="rz-user-random-once"
            label="روم عشوائي مرّة واحدة"
            action={() => {
                void moveUserToRandomChannelOnce(user);
            }}
        />
    );

    // تمشية عشر رومات فوق/تحت (حسب الإعداد)
    groups.push(
        <Menu.MenuItem
            id="rz-user-offset-up"
            label={`نقله فوق ${settings.store.voiceTourOffset} رومات`}
            action={() => {
                void moveUserOffset(user, "up");
            }}
        />
    );
    ...
    groups.push(
        <Menu.MenuItem
            id="rz-user-offset-down"
            label={`نقله تحت ${settings.store.voiceTourOffset} رومات`}
            action={() => {
                void moveUserOffset(user, "down");
            }}
        />
    );

    if (voiceChannel && voiceChannel.guild_id && !isBot(user.id)) {
        const canMuteHere = PermissionStore.can(MUTE, voiceChannel);

        if (canMuteHere) {
            groups.push(
                <Menu.MenuItem
                    id="rz-user-server-mute"
                    label="Server Mute هذا الشخص"
                    action={() => {
                        void serverMuteUser(
                        ...
                        showRZMessage(
                            `تم Server Mute لـ ${user.username}`,
                            "success"
                        );
                    }}
                />
            ...
            groups.push(
                <Menu.MenuItem
                    id="rz-user-server-unmute"
                    label="Server Unmute هذا الشخص"
                    action={() => {
                        void serverMuteUser(
                            voiceChannel.guild_id!,
                            user.id,
                        ...
                        showRZMessage(
                            `تم Server Unmute لـ ${user.username}`,
                            "success"
                        );
                    }}
                />
        ...
        const label = blacklisted
            ? "شيل بلاك من هذا الروم"
            : "حط بلاك في هذا الروم";
        ...
        groups.push(
            <Menu.MenuItem
                ...
                icon={BlockIcon}
                action={() => {
                    const chId = voiceChannel.id;
                    const gId = voiceChannel.guild_id!;
                    if (!hasPermsBlacklist(chId, "any")) {
                        showRZMessage(
                            "ما عندك صلاحيات ميوت/دفن/نقل في هذا الروم",
                            "error"
                        );
                        return;
                    }
                    ...
                    if (blacklisted) {
                        removeBlacklist(user.id, chId);
                        showRZMessage(
                            "تم إزالة الشخص من البلاك ليست الصوتية لهذا الروم",
                            "success"
                        );
                    } else {
                        addBlacklist(user.id, chId);
                        showRZMessage(
                            "تم إضافة الشخص للبلاك ليست الصوتية لهذا الروم",
                            "success"
                        );
                        const vs =
                            VoiceStateStore.getVoiceStatesForChannel(chId)[
                                user.id
                            ];
                        if (vs?.channelId === chId) {
                            kickUserWithSettings(gId, user.id, chId);
                        }
                    }
                }}
            />
...
const VoiceChannelContext: NavContextMenuPatchCallback = (
    ...
    if (followIds.length) {
        ...
        items.push(
            <Menu.MenuItem
                key="follow-clear"
                id="follow-clear"
                label="مسح جميع الفولو"
                color="danger"
                action={() => {
                    Alerts.show({
                        title: "مسح الفولو",
                        body: `راح يتم مسح قائمة الفولو بالكامل (${followIds.length})`,
                        confirmText: "مسح",
                        cancelText: "إلغاء",
                        confirmColor: "red",
                        onConfirm: () => {
        ...
        groups.push(
            <Menu.MenuItem key="follow-list" id="follow-list" label="Follow">
                {items}
            </Menu.MenuItem>
        );
    }
            ...
            <Menu.MenuItem
                ...
                action={() => {
                    Alerts.show({
                        ...
                        body: `راح تمسح قائمة السحب بالكامل (${pullIds.length})`,
                        confirmText: "مسح",
                        cancelText: "إلغاء",
                        confirmColor: "red",
                        onConfirm: () => {
            ...
            <Menu.MenuItem
                ...
                action={() => {
                    Alerts.show({
                        ...
                        body: `راح تفك التجميد عن ${frozenIds.length} شخص`,
                        confirmText: "فك الكل",
                        cancelText: "إلغاء",
                        confirmColor: "green",
                        onConfirm: () => {
    ...
    if (blkList.length && hasPermsBlacklist(channel.id, "any")) {
    ...
    /* Voice Tools + Mention */
    const canVoiceMute = hasPermsBlacklist(channel.id, "mute");
    const canVoiceDeaf = hasPermsBlacklist(channel.id, "deaf");
    const canVoiceMove = hasPermsBlacklist(channel.id, "move");
        ...
        >
            ...
            <Menu.MenuItem
                ...
                action={() => {
                    ...
                    sendPatchVoiceTools(channel, { channel_id: null }, true);
                }}
            />
            <Menu.MenuItem
                label="Move all"
    ...
    /* قائمة الناس اللي قاعدين يمشّون في هذا السيرفر */
    const touringUsers = Array.from(tourRunningUsers).filter(uid => {
        const chId = getUserChannelId(uid);
        if (!chId) return false;
        const ch = ChannelStore.getChannel(chId);
        return ch?.guild_id === channel.guild_id;
    });

    if (touringUsers.length) {
        const items = touringUsers.map(uid => {
            ...
            return (
                <Menu.MenuItem
                    key={uid}
                    id={`tour-stop-${uid}`}
                    label={`إيقاف تمشية ${name}`}
                    action={() => {
                        tourRunningUsers.delete(uid);
                        delete tourOriginalChannel[uid];
                        showRZMessage(
                            `تم إيقاف التمشية عن ${name}`,
                            "info"
                        );
                    }}
                />
    ...
    // زر "روم عشوائي" عليك أنت من منيو الروم
    groups.push(
        <Menu.MenuItem
            key="rz-me-random-once"
            id="rz-me-random-once"
            label="روم عشوائي (عليّ أنا)"
            action={() => {
                void moveMeToRandomChannelOnceFromChannelMenu(channel);
            }}
        />
    );
    ...
    VOICE_STATE_UPDATES({ voiceStates }: { voiceStates: VoiceState[] }) {
        ...
        for (const vs of voiceStates as any) {
            ...
            if (
                ...
                const ch = ChannelStore.getChannel(channelId);
                if (ch && hasPermsBlacklist(channelId, "any")) {
            ...
            // Pull List + Auto Role Pull لما تتحرك أنت
            if (
                isMe &&
                channelId &&
                ...
                // سحب قائمة السحب تلقائيًا إلى رومك الحالي (لو الخيار شغال)
                if (settings.store.autoPullListOnMove) {
                    const ids = getPullListIds().filter(id => !isBot(id));
                    for (const id of ids) {
                ...
                // Auto Role Pull: سحب أصحاب الرول المحدد تلقائيًا إلى رومك الحالي
                if (settings.store.autoRolePullEnabled && settings.store.pulledRoleId) {
                    void pullRoleMembersToChannel(
                        gId,
                        settings.store.pulledRoleId,
                        channelId
